...
export function printReport(report: AuditReport, triage?: TriageResult, suppressedCount?: number): void {
  ...
  console.log('');
  console.log(`  ${c.dim}© 2026 resetroot99 & ajakvani — https://github.com/resetroot99/FLAW${c.reset}`);
  console.log(`  ${c.dim}Licensed under BSL 1.1. Commercial use requires a license.${c.reset}`);
  console.log('');
}
...
export function exportMarkdown(report: AuditReport, outputDir: string): string {
  ...
  lines.push(`*© 2026 resetroot99 & ajakvani — [FLAW](https://github.com/resetroot99/FLAW) — BSL 1.1*`);
